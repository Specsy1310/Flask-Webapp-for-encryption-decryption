...
def readEncryptedKeys():
    target_file = open(f"raw_data/{iddd}/store_in_me.enc", "rb")
    encryptedKeys = b""
...
def readEncryptedText(filename):
    source_filename = f'encrypted/{iddd}/' + filename
...
def decrypter():
    ...
    files = sorted(tools.list_dir(f'encrypted/{iddd}/'))
