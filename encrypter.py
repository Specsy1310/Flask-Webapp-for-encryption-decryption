...
def writeEncryptedText(filename, encryptedData: bytes):
    ...
    target_filename = f'encrypted/{idd}/' + filename
...
def writeEncryptedKeys(encryptedKeys: bytes):
    ...
    target_file = open(f"raw_data/{idd}/store_in_me.enc", "wb")
...
def encrypter():
    ...
    public_key = open(f"./key/Main_Key{idd}.pem", "wb")
